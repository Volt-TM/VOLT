...
exports.run = async (client, message, args) => {
  if(!message.member.hasPermission(['KICK_MEMBERS'])) return message.reply("you dumb af") ;
  ...
  if (message.guild.member(user).roles.cache.has(muteRole.id)) {
    ...
    message.guild.member(user).roles.remove(muteRole).then(() => {
      ...
      let logchannel = message.guild.channels.cache.find(channel = channel.name === 'envo-logs');
      if  (!logchannel){
